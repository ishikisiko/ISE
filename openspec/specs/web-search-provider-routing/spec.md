...
> **Status:** active - roadmap M5 provider eligibility contract.
...
### Requirement: Web provider routing SHALL execute only eligible provider attempts
系统 SHALL 根据用户显式 source 选择和已配置 provider 库存执行网页 provider，而不是让隐式特殊分支绕过这些约束。

#### Scenario: Default priority search is eligible
- **WHEN** 模型调用网页搜索工具且用户未限制来源
...
- **THEN** 搜索工具 SHALL 将该子集作为 provider 约束
...
系统 SHALL 将 provider 库存和每个实际工具调用的尝试分开暴露。
...
For a target-domain search tool call, the system SHALL continue through the
...
- **AND** it SHALL attempt the next eligible provider in deterministic order
