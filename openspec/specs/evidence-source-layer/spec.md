...
#### Scenario: Registered skill retrieval is represented as an EvidenceSource
- **WHEN** 查询需要 weather、finance、sports 或其他 registered skill 提供的结构化证据
...
#### Scenario: Skill-assisted query enables multiple source types
- **WHEN** registered skill 返回需要继续搜索或继续补证据的结果
- **THEN** 默认主编排器 SHALL 能同时启用 `domain` 和其他来源类型
- **AND** 系统 SHALL 不要求 skill 数据只能旁路注入字符串上下文

### Requirement: Response metadata SHALL expose active evidence source semantics
系统 SHALL 暴露统一的来源元数据语义，以说明哪些工具可用、哪些实际执行，以及哪些独立证据引用被最终使用。

#### Scenario: Default pipeline reports eligible and executed evidence sources
- **WHEN** 默认主链路完成一次回答
- **THEN** 返回结果 SHALL 标识循环可用的一级来源类型和来源标识
...
#### Scenario: Recovery tool reports reused evidence source types
- **WHEN** 循环中的高层恢复工具复用统一来源层
- **THEN** 返回结果或工具输出 SHALL 能说明复用了哪些来源类型和最终证据引用
- **AND** 该元数据 SHALL 与默认主链路使用兼容的来源语义

### Requirement: EvidenceSource output SHALL preserve tool and policy provenance
系统 SHALL 让归一化证据携带来源层级、实际工具调用和可用于策略验证的元数据，而不改变其通用 `EvidenceItem` 接口。
...
#### Scenario: Skill or local result enters the ledger
- **WHEN** registered skill 或本地文档结果被归一化为 `EvidenceItem`
- **THEN** 系统 SHALL 保留其来源身份和产生它的实际工具调用
- **AND** 系统 SHALL 允许其与网页证据共同满足分析约束
...
official domain for an entity in the current query analysis, and SHALL preserve the matched
