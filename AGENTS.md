...
│   └── styles.css
├── uploads/                 # 上传文档目录
├── checkpoints/             # 会话 checkpoint 与记录（SQLite，按 LRU 清理；gitignored）
├── config.json              # 配置文件
└── requirements.txt         # Python 依赖
```

The default runtime path is the LangChain-based orchestrator assembled by `main.py` and `server.py`, with legacy compatibility code still available behind explicit switches. Search clients live in `search/search.py`, the default orchestration and RAG execution live under `langchain/`, and persisted uploads land in `uploads/`; clean this directory when rotating documents. Conversation state (LangGraph checkpoints + per-turn records) is persisted under `checkpoints/conversations.sqlite` (see the `conversation` config block; disable by setting `conversation.enabled=false` to fall back to stateless single-turn behaviour); clean this directory to reset all sessions. Configuration secrets belong in `config.json` (copy from `config.example.json`) or an alternate path exposed via `NLP_CONFIG_PATH`. The optional `orchestration.reformulation_recovery` block controls bounded deterministic re-search and defaults to enabled; set `enabled=false` to preserve single-retrieval behavior. Use `orchestration.official_domains` (or `official_domain_resolution.pins`) to map normalized entity stems to official registrable domains, which take precedence over first-party domain-stem matching. Official-domain ruling (`evidence/official_domain_resolver.py` + `evidence/official_domain_graph.py`) is a deterministic function of a cached relation graph (registry↔homepage↔repo closure, trusted redirects, cert SAN, facades, CT-log age; search voting is only a weak signal), so rule changes re-adjudicate cached graphs offline. Pins skip validation by design and are re-checked by `pin_shadow_audit` (never overridden, disagreements surfaced in the resolution audit). `official_domain_resolution.agent_discovery` enables agent-assisted discovery for candidate/none entities — the agent only fills the relation graph; the verdict stays with the resolver. The `autonomy` block is the carrier of loop rule strength: `autonomy.mode` (`guided`, the default, reproduces pre-capability behaviour; `autonomous` widens budgets and relaxes binding rules so the model can lead open-ended tasks) and `autonomy.profiles.<mode>` for local overrides. Resolution priority is request > config > default `guided`. `preflight` (skill argument pre-checks) is always binding in both modes — the policy offers no value that disables it. The `termination` block remains the budget source for `guided` (so the default stays equivalent to the configured budget); `autonomous` budgets live in `autonomy.profiles.autonomous.budgets`. This is a long-term capability switch, not a migration flag (see `docs/agentic_loop_roadmap.md` M6).

## Build, Test, and Development Commands
Install dependencies once per virtual environment: `pip install -r requirements.txt`. Launch the web app with `python server.py` and visit `http://localhost:8000`. The CLI entrypoint stays available through `python main.py "Your question"`. Use `python main.py "Your question" --search off --data-path ./uploads` for local-doc-only retrieval, or keep search enabled and pass `--data-path` to combine uploaded/local docs with live search. Use `--provider` to override the default LLM for a single run. Use `--autonomy {guided,autonomous}` to override the effective autonomy mode for a single run (defaults to `config.json`'s `autonomy.mode`, then `guided`); the resolved mode and its source are echoed in the response `control.autonomy`. For multi-turn follow-ups, pass `--conversation-id <id>` (or omit it; the printed id can be reused on the next call to refine the previous answer). The web UI carries a `conversation_id` automatically; the "新会话" pill starts a fresh conversation. When debugging configuration, set `NLP_CONFIG_PATH=/full/path/config.json` before invoking either entrypoint.
